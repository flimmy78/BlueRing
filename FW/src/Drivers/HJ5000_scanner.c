...
*/
static int UE988_set_codeParam(unsigned char *param, unsigned char param_len)
{
	...
	len = pack_command(0xC6, 0x08, req_data, param_len+1);
...
void scanner_mod_init(void)
{
	...
	//UE988_set_codeParam("\x20\x0a\x01",3);
	wait_time_out = 28;
	//scan_start = 0;
}
...
int scanner_RxISRHandler(unsigned char *c,int len)
{
  unsigned int i;
#if(USART_RX_MODE == USART_RX_DMA_MODE)
	if(g_resCmd.status == 0x5a)
	{
		memcpy(g_resCmd.CmdBuffer,c,len);
		g_resCmd.CmdPos = len;
		g_resCmd.status = RESPONSE_SUCCESS;
		SCANNER_TRIG_OFF();
		OSQPost(pEvent_Queue,(void*)EVENT_SCAN_GOT_BARCODE);
		i = ((g_resCmd.CmdPos > MAX_BARCODE_LEN)?MAX_BARCODE_LEN:g_resCmd.CmdPos);
